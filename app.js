...
client.on("message", async (msg) => {
    ...
    if(msg.content.toLowerCase() === "m.ping"){
        let message = await msg.channel.send(`🐱 Checking the ping... one moment.`);
        return message.edit(`🙀 My ping is: \`${message.createdTimestamp - msg.createdTimestamp}\`ms, API Latency: \`${Math.round(client.ws.ping)}\`ms.`)
    }
})
...
app.post("/upvote/bfd", async (req, res) => {
    ...
    let vote = await handleVote(user, {
        title: "Vote Here",
        url: `https://botsfordiscord.com/bot/688542620040953913/vote`,
        color: 0x2578fe,
        timestamp: new Date(),
        author: {
            name: `New Vote By: @${user.tag}`,
